...
%>
	<tr id="<%=dto.getRm() %>">
		...
		<td>
		<a href="/member.do?method=getListDetail&user_id=<%=dto.getId()%>&cate=<%=request.getParameter("cate")%>&gubun=<%=request.getParameter("gubun")%>">
		<%=dto.getId()%>
		<!-- <%if (dto.getProfile().length() > 80) {%>
			<%=common.cutString(dto.getProfile(),80) %>...
		<%} else { %>
			<%=dto.getProfile()%>
		<%} %>-->
		</a>
		...
		<td><%=dto.getDone_cnt() %></td>
		<td><%=dto.getSelected_cnt() %></td> --%>
	</tr>
	
<% 	}
} else {
%>
<tr id="0"><td colspan="7">더이상 목록이 없습니다.</td></tr>
<%	
}	
%>
